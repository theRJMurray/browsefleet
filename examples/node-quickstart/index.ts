...
async function main() {
  ...
  const health = await healthRes.json() as { status: string; activeSessions: number; maxSessions: number };
  ...
  const scrape = await scrapeRes.json() as { url: string; statusCode: number; title: string; markdown: string };
  ...
  try {
    ...
    if (!actionsRes.ok) throw new Error(`Actions failed: ${actionsRes.status} ${await actionsRes.text()}`);
    const actions = await actionsRes.json() as { results: Array<{ type: string; success: boolean }> };
    console.log('  Action results:', actions.results.map(r => `${r.type}=${r.success}`).join(', '));
  } finally {
